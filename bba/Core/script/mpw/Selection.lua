--------------------------------------------------------------------------------
-- formations for cavalry
-- switched highlighting of serf and building menu buttons
-- set and show troll expel button
-- version checks
-- fix battleserf formation bug
--------------------------------------------------------------------------------
function GameCallback_GUI_SelectionChanged()
	...
	for i,id in ipairs( GUI_Selection ) do
		
		local entitytype = Logic.GetEntityType( id )
		
		if entitytype == Entities.PB_Tower2_Ballista or entitytype == Entities.PB_Tower3_Cannon then
			table.insert( turrets, id )
		end
	end
	
	if table.getn( GUI_Selection ) > 0 then
		
		-- only turrets selected ?
		if table.getn( GUI_Selection ) == table.getn( turrets ) then
			
			-- select tower of first turret
			GUI.SetSelectedEntity( CEntity.GetAttachedEntities( turrets[1] )[55][1] )
			return
		else
			-- remove all turrets from selection
			for _,id in ipairs( turrets ) do
				GUI.DeselectEntity( id )
			end
			
			if table.getn( turrets ) > 0 then
	...
	elseif GUI_Selection == nil or table.getn(GUI_Selection) <= 1 then
	...
	elseif Logic.IsLeader( EntityId ) == 1 then
		...
		--this needs to be called for resetting the expell button for heros
		XGUIEng.ShowWidget("Commands_HeroContainer",0)
		...
		--Scout and Thief
		if Entities.PU_Scout and EntityType == Entities.PU_Scout then
		...
		if Entities.PU_Thief and EntityType == Entities.PU_Thief then
		...
		if GUI_Selection[2] == nil then
			...
			if Logic.IsHero(EntityId) == 1 then
				...
				XGUIEng.ShowWidget("Commands_HeroContainer",1)
				XGUIEng.SetWidgetPosition("Command_ExpelHero",287,37)
				...
				elseif EntityCategories.Hero10 and Logic.IsEntityInCategory(EntityId,EntityCategories.Hero10) == 1 then
					FunnyComment = Sounds.AOVoicesHero10_HERO10_FunnyComment_rnd_01
				elseif EntityCategories.Hero11 and Logic.IsEntityInCategory(EntityId,EntityCategories.Hero11) == 1 then
					FunnyComment = Sounds.AOVoicesHero11_HERO11_FunnyComment_rnd_01
				elseif Entities.CU_Evil_Queen and EntityType == Entities.CU_Evil_Queen then
			...
			--disbale Buy soldier Area for cannons
				...
				or (Entities.PU_Scout and EntityType == Entities.PU_Scout)
				or (Entities.PU_Thief and EntityType == Entities.PU_Thief) then
	...
	elseif Logic.IsBuilding( EntityId ) == 1 then
		...
		if Logic.IsConstructionComplete( EntityId ) == 1 then
			...
			elseif UpgradeCategories.Tavern and UpgradeCategory == UpgradeCategories.Tavern then
			...
			elseif UpgradeCategories.MasterBuilderWorkshop and UpgradeCategory == UpgradeCategories.MasterBuilderWorkshop then
